...
if (isset($_POST['submit'])) {
    ...
    // Check for at least one number
        ...
        header("Location: resetPwdForm.html?success=8");
            ...
            $sql = "SELECT * FROM users WHERE email = ? AND ChangePwdEmailCode = 'Pending' AND ChangePwdbackupEmailCode = 'Pending'";            
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE users SET passwords = ?, ChangePwdEmailCode = 0, ChangePwdbackupEmailCode = 0 WHERE email = ? AND ChangePwdEmailCode = 'Pending' AND ChangePwdbackupEmailCode = 'Pending'";
            $stmt = $conn->prepare($sql);
