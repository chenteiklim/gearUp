...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['deleteRequest']) && !isset($_POST['cancelDeleteRequest'])) {
    ...
    $updateQuery = "UPDATE users SET email=?, address=? WHERE user_id=?";
    $stmt = $conn->prepare($updateQuery);
    $stmt->bind_param("ssi", $email, $encrypted_address, $UserId);
...
<form id="profileForm" method="POST" action="profile.php">
    ...
    <input type="text" id="username" value="<?= htmlspecialchars($user['usernames']) ?>" readonly>
    ...
    <?php else: ?>
        <button id="deleteBtn" type="submit" name="deleteRequest"
