...
$username = $_SESSION['sellerUsername'] ?? null;
echo $username;
if (!$email || !$username) {
...
if ($result4->num_rows > 0) {
    ...
    // Clean up fake or redundant pending account using same email
    $delete_stmt = $conn->prepare("
        DELETE FROM users 
        WHERE email = ? AND status = 'pending' AND user_id = ?
    ");
    $delete_stmt->bind_param("si", $email, $user_id);
    $delete_stmt->execute();
    $delete_stmt->close();

    header("Location: ../login/login.php?success=2"); // Email already in use
    exit;
}
$stmt4->close();
