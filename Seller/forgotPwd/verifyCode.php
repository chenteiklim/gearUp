...
$sql = "SELECT ChangePwdEmailCode, ChangePwdbackupEmailCode FROM seller WHERE email = ? AND backupEmail = ?";
...
if ($stmt->num_rows > 0) {
    ...
    if (password_verify($primaryCode, $hashedEmailCode) && password_verify($backupCode, $hashedBackupEmailCode)) {
        ...
        $updateSql = "UPDATE seller SET ChangePwdEmailCode = 'pending', ChangePwdbackupEmailCode = 'pending' WHERE email = ? AND backupEmail = ?";
